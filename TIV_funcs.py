...
def TIV_model_Baccam(param, max_time):  # Where param = [pV, beta]
...
def TIV_ll_Baccam(V_data, param, max_time):  # Where I_data = I (infected individuals) as retrieved from data

    ll = 0

    # TODO: Try to reject unsolved parameter solutions. Should theoretically stop throwing up IndexError
    if TIV_model_Baccam(param, max_time).success == False:  #Reject parameter if solve_ivp failed
        ll = -1 * np.inf
        print('solve_ivp failed for param ' + str(param) + ' so rejecting value')
        return ll
